...
export function createFakeSTT(options?: {
  ...
  return {
    ...
    emitTranscript: (text, isFinal) => {
      if (!ctx) throw new Error("STT context not initialized");
...
export function createFakeLLM(): FakeLLM {
  ...
  const requireContext = (): LLMContext => {
    if (!ctx) throw new Error("LLM context not initialized");
    return ctx;
  };
...
export function createFakeVAD(): FakeVAD {
  ...
  return {
    ...
    emitSpeechStart: () => {
      if (!ctx) throw new Error("VAD context not initialized");
      ctx.speechStart();
    },
...
export function createFakeTurnDetector(): FakeTurnDetector {
  ...
  return {
    ...
    emitTurnEnd: (transcript) => {
      if (!ctx) throw new Error("Turn detector context not initialized");
